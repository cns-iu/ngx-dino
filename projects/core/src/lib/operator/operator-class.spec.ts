import { isOperator, Operator } from './operator-class';
...
describe('Operator', () => {
  ...
  describe('new Operator(op1, op2, ...)', () => {
    ...
    it('accepts a mix of argument types', () => {
      expect(() => new Operator(fun1, operator1, [fun2])).not.toThrow();
    });

    it('throws on invalid argument types', () => {
      expect(() => new Operator(1 as any)).toThrow(jasmine.any(TypeError));
    });
  });
  ...
  describe('.pipe(op1, op2, ...)', () => {
    ...
    it('returns this if no new functions are added', () => {
      expect(operator1.pipe(operator1)).toEqual(operator1);
    });
  });
});
